...
| MM-001 | Done | Party / MMSession | Strengthen Party to MM binding | `BigRunTest_02.txt` showed Party attrs `MatchmakingSessionID_s` / `GameSessionID_s` as stronger evidence than the member-set heuristic. Projector now prefers explicit Party -> MMSession binding and keeps the heuristic as fallback. Validated by Archy in Win11 environment on 2026-06-18. |
